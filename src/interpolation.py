...
def electric_field_pond(field, x, L):
	alpha = x - math.floor(x)
	return alpha * field[math.floor(x)+1] + (1-alpha) * field[math.floor(x)] 

# renvoie le vecteur contenant les nouvelle vitesse des particules
def calc_vit_pond(h, nbr_particules, pos_old, vit_old, L, field):
	vit_new = zeros(nbr_particules)
	for i in range(nbr_particules):
		vit_new[i] = vit_old[i] - h*(qe/me)*electric_field_pond(field,pos_old[i],L)
	return vit_new
